...
| MQTT topics show gateway `0000000000000000` | boot race on `MQTT_BACKEND=mesh` gateways: the mqtt-forwarder read the gateway ID from the mesh proxy before it was known, and it only reads once. **Restart gateway-mesh first, then mqtt-forwarder** — the mesh's own backend can also be stuck at zeros (its concentratord command loop does not re-fetch the ID after an early failure), in which case restarting only the mqtt-forwarder reproduces the zeros. Reported upstream and **fixed on master 2026-08-06**: [chirpstack-gateway-mesh#130](https://github.com/chirpstack/chirpstack-gateway-mesh/issues/130) (commit `f07f11c` — backend now set up before the proxy, so the command socket only exists once the real ID is cached) and [chirpstack-mqtt-forwarder#73](https://github.com/chirpstack/chirpstack-mqtt-forwarder/issues/73) (commit `c2049f7` — an all-zero ID is rejected and retried, same approach as our ttn-forwarder v0.2.1). Neither fix is in a tagged release yet (they postdate gateway-mesh v4.1.3 / mqtt-forwarder v4.6.0); until the pinned artifact versions are bumped, the startup gates in the entrypoints (release rev13+) remain the active protection |
...
| `SHA256_MQTT_FWD` | `e7d3041084e30e1fe89117f0ce0db6f5f626324562bfffd76a2257f8a473eb52` | `2aa3d4b6d36020354f0997ea634eebdd2b0cffccfd57d79846fdebe4cc55a398` |

(Versions pinned: concentratord 4.7.1, mqtt-forwarder 4.6.1. For
gateway-mesh on arm64, override `SHA256_MESH` in
[gateway-mesh/Dockerfile.template](../gateway-mesh/Dockerfile.template) with
`f29e7b76b05acf819991cd2cbbae12f97576d8e30a8f35b019318992b4e404fb` (v4.1.4).)
